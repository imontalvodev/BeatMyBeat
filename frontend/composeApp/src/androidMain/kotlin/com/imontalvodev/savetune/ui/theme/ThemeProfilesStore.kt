...
class ThemeProfilesStore(private val context: Context) {
    private val prefs = context.getSharedPreferences("savetune_theme_profiles", Context.MODE_PRIVATE)

    fun loadProfiles(): List<SavetuneThemeProfile> {
        ...
        val out = mutableListOf<SavetuneThemeProfile>()
        for (i in 0 until arr.length()) {
            val o = arr.optJSONObject(i) ?: continue
            out += SavetuneThemeProfile(
    ...
    fun saveProfiles(profiles: List<SavetuneThemeProfile>) {
        val arr = JSONArray()
        profiles.forEach { p ->
            arr.put(
                JSONObject().apply {
    ...
    fun defaultProfiles(): List<SavetuneThemeProfile> = listOf(NeonMintProfile, CherryPulseProfile)
